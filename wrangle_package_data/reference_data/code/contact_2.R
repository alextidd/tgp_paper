...
contactDir <- "output/contact/"
replicatesDir <- paste0(contactDir, "replicates/")
byChrDir <- paste0(contactDir, "by_chr/")
preQCDir <- paste0(contactDir, "pre_QC/")
postQCDir <- paste0(contactDir, "post_QC/") ; dir.create(postQCDir)
...
for(file in files){ 
  ...
  contact[[info$assay]][[info$celltype]] <- df
}

# save
saveRDS(contact, file = "output/contact/contact.rds")
